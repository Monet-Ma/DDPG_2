...
from WebSimEnv import WebSimEnv
...
guidpfrom = 0.5
guidpto = 0.05
actionexploration = LinearSchedule(schedule_timesteps=12,
                                            initial_p=guidpfrom,
                                            final_p=guidpto)
...
class DDPG(object):
    ...
    def save_ckpt(self):
        ...
        tl.files.save_weights_to_hdf5('model5/ddpg_actor.hdf5', self.actor)
        tl.files.save_weights_to_hdf5('model5/ddpg_actor_target.hdf5', self.actor_target)
        tl.files.save_weights_to_hdf5('model5/ddpg_critic.hdf5', self.critic)
        tl.files.save_weights_to_hdf5('model5/ddpg_critic_target.hdf5', self.critic_target)
    ...
    def load_ckpt(self):
        ...
        :return: None
        """
        tl.files.load_hdf5_to_weights_in_order('model5/ddpg_actor.hdf5', self.actor)
        tl.files.load_hdf5_to_weights_in_order('model5/ddpg_actor_target.hdf5', self.actor_target)
        tl.files.load_hdf5_to_weights_in_order('model5/ddpg_critic.hdf5', self.critic)
        tl.files.load_hdf5_to_weights_in_order('model5/ddpg_critic_target.hdf5', self.critic_target)
...
if __name__ == '__main__':
    ...
    # 初始化环境
    jarpath = os.path.abspath('.') + '\WebSimulator2.10.jar'
